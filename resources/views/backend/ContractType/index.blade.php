...
<div class="row">
    <div class="col-12">
        <div class="page-title-box d-flex align-items-center justify-content-between">
            <h4 class="mb-0">Contract Type</h4>
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="modalLabel">Add New Contract Type</h5>
...
<div class="modal fade" id="importModal" tabindex="-1" aria-labelledby="importModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="importModalLabel">Import Contract Types from Excel</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body">
...
$(document).ready(function() {
    ...
    function fetchData(page = 1, search = '') {
        ...
        $.ajax({
            url: "{{ route('typeOfContactList.index') }}",
            type: 'GET',
    ...
    $('#add_button').on('click', function() {
        clearForm();
        $('#modalLabel').text('Add New Contract Type');
        $('#submit_button').text('Save');
        $('#form_method').val('POST');
        $('#modalForm').attr('action', "{{ route('typeOfContactList.store') }}");
        $('#formModal').modal('show');
    });

    $(document).on('click', '.edit-button', function() {
        let id = $(this).data('id');
        let url = "{{ route('typeOfContactList.edit', ':id') }}".replace(':id', id);
        $.get(url, function(data) {
            clearForm();
            $('#modalLabel').text('Edit Contract Type');
            $('#submit_button').text('Update');
            $('#form_method').val('PUT');
            $('#modalForm').attr('action', "{{ route('typeOfContactList.update', ':id') }}".replace(':id', data.id));
            $('#id').val(data.id);
            $('#name').val(data.name);
            $('#formModal').modal('show');
        });
    ...
    $(document).on('click', '.delete-button', function() {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                let url = "{{ route('typeOfContactList.destroy', ':id') }}".replace(':id', id);
                $.ajax({
                    url: url,
                    type: 'POST',
    ...
    $('#importForm').on('submit', function(e) {
        ...
        $.ajax({
            url: "{{ route('typeOfContactList.import') }}",
            type: 'POST',
            data: formData,
            contentType: false,
            processData: false,
    ...
    $('#delete_selected_button').on('click', function() {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: "{{ route('typeOfContactList.deleteMultiple') }}",
                    type: 'POST',
                    data: {
                        _token: "{{ csrf_token() }}",
                        ids: selectedIds
