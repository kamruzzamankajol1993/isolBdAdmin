...
<a href="{{ route('eventList.create') }}" class="btn btn-primary dropdown-toggle waves-effect  btn-sm waves-light" type="button" >
                                        <i class="far fa-calendar-plus  mr-2"></i> Add Event Detail
</a>
                        ...
                        <div class="col-12">
                            <div class="card">
                                ...
                                <tr>
                                   <td>{{ $loop->index+1 }}</td>
                                   <td>{{$user->title }}</td>
                                   <td>{{$user->date }}<br>Time:{{$user->time}} </td>
                                   <td>{{$user->speaker }}</td>
                                   <td>
                                    ...
                                    <td>
                                        ...
                                        @if (Auth::guard('admin')->user()->can('eventView'))
                                        <a type="button" href="{{ route('eventList.show',$user->id) }}"
                                          class="btn btn-success waves-light waves-effect  btn-sm" >
                                          <i class="fas fa-list"></i></a>
                                          
                                          
                                          <!-- Button trigger modal -->
<button type="button" class="btn btn-info btn-sm" data-bs-toggle="modal" data-bs-target="#exampleModal{{$user->id}}">
...
<button   type="button" class="btn btn-danger waves-light waves-effect  btn-sm" onclick="deleteTag({{ $user->id}})" data-toggle="tooltip" title="Delete"><i class="fas fa-trash-alt"></i></button>
                    <form id="delete-form-{{ $user->id }}" action="{{ route('eventList.destroy',$user->id) }}" method="POST" style="display: none;">
@method('DELETE')
                                                    @csrf
