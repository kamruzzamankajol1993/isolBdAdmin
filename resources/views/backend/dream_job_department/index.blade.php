            ...
            <div class="modal-body">
                <form id="importForm" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="mb-3">
                        ...
                        <div class="mt-2">
                             <a href="{{ asset('public/samples/sample-dp-import-file.xlsx') }}" download>Download Sample File</a>
...
$(document).ready(function() {
    ...
    function fetchData(page = 1, search = '') {
        ...
        $.ajax({
            url: "{{ route('dreamJobDepartmentList.index') }}",
            type: 'GET',
    ...
    $('#add_button').on('click', function() { /* ... existing code ... */ });
    $(document).on('click', '.edit-button', function() { /* ... existing code ... */ });
    $('#modalForm').on('submit', function(e) { /* ... existing code ... */ });
    $(document).on('click', '.delete-button', function() { /* ... existing code ... */ });
    ...
    $('#importForm').on('submit', function(e) {
        ...
        $.ajax({
            url: "{{ route('dreamJobDepartmentList.import') }}",
            type: 'POST',
            data: formData,
            contentType: false,
            processData: false,
    ...
    $('#delete_selected_button').on('click', function() {
        ...
        Swal.fire({
            title: 'Are you sure?',
            text: `You are about to delete ${selectedIds.length} selected departments.`,
            icon: 'warning',
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: "{{ route('dreamJobDepartmentList.deleteMultiple') }}",
                    type: 'POST',
                    data: {
                        _token: "{{ csrf_token() }}",
                        ids: selectedIds
                    ...
                    error: function(xhr) {
                        Swal.fire('Error!', 'Could not delete the selected departments.', 'error');
                    }
                });
            }
        });
    ...
    // --- Paste existing JS logic for add, edit, single delete here to keep it concise ---
    $('#add_button').on('click', function() {
        clearForm();
        $('#modalLabel').text('Add New Department');
        $('#submit_button').text('Save');
        $('#form_method').val('POST');
        $('#modalForm').attr('action', "{{ route('dreamJobDepartmentList.store') }}");
        $('#formModal').modal('show');
    });

    $(document).on('click', '.edit-button', function() {
        let id = $(this).data('id');
        let url = "{{ route('dreamJobDepartmentList.edit', ':id') }}".replace(':id', id);
        $.get(url, function(response) {
            if (response && response.id) {
                ...
                $('#form_method').val('PUT');
                let actionUrl = "{{ route('dreamJobDepartmentList.update', ':id') }}".replace(':id', response.id);
    ...
    $(document).on('click', '.delete-button', function() {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                let url = "{{ route('dreamJobDepartmentList.destroy', ':id') }}".replace(':id', id);
                $.ajax({
                    url: url,
                    type: 'POST',
