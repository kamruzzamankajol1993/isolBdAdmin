...
$(document).ready(function() {
    ...
    // Function to render professional pagination
    function renderPagination(response) {
        ...
        paginationLinks.append(`<li class="page-item ${currentPage === 1 ? 'disabled' : ''}"><a class="page-link" href="#" data-page="${currentPage - 1}">Previous</a></li>`);
        const window = 1;
        let pages = [...new Set([1, ...Array.from({length: Math.min(lastPage - 1, currentPage + window) - Math.max(2, currentPage - window) + 1}, (_, i) => Math.max(2, currentPage - window) + i), lastPage > 1 ? lastPage : 1])];
        let lastAddedPage = 0;
        for (const page of pages) {
            ...
            paginationLinks.append(`<li class="page-item ${currentPage === page ? 'active' : ''}"><a class="page-link" href="#" data-page="${page}">${page}</a></li>`);
            lastAddedPage = page;
        }
        paginationLinks.append(`<li class="page-item ${currentPage === lastPage ? 'disabled' : ''}"><a class="page-link" href="#" data-page="${currentPage + 1}">Next</a></li>`);
    }
    ...
    // --- Custom Select Logic ---
    $(document).on('click', '.custom-select-value', function(e) {
        e.stopPropagation();
        $('.custom-select-options').not($(this).next()).hide();
        $(this).next('.custom-select-options').toggle().find('.custom-select-search').focus();
    });
    $(document).on('click', '.custom-select-list li', function() {
        let container = $(this).closest('.custom-select-container');
        let value = $(this).data('value');
        let text = $(this).text();
        container.find('.custom-select-value').text(text).removeClass('is-invalid');
        container.next('select').val(value).trigger('change');
        container.find('.custom-select-options').hide();
    });
    $(document).on('keyup', '.custom-select-search', function() {
        let filter = $(this).val().toUpperCase();
        $(this).next('.custom-select-list').find('li').each(function() {
    ...
    $('#add_button').on('click', function() {
        $('#modalLabel').text('Add New Position');
        $('#submit_button').text('Save');
        $('#form_method').val('POST');
        $('#modalForm').attr('action', "{{ route('dreamJobPositionList.store') }}");
        $('#formModal').modal('show');
    });

    $(document).on('click', '.edit-button', function() {
        let id = $(this).data('id');
        let url = "{{ route('dreamJobPositionList.edit', ':id') }}".replace(':id', id);
        $.ajax({
            url: url,
            type: 'GET',
            success: function(response) {
                if (response && response.id) {
                    $('#modalLabel').text('Edit Position');
                    $('#submit_button').text('Update');
                    $('#form_method').val('PUT');
                    let actionUrl = "{{ route('dreamJobPositionList.update', ':id') }}".replace(':id', response.id);
                    $('#modalForm').attr('action', actionUrl);
                    $('#id').val(response.id);
                    $('#name').val(response.name);
                    // Set value for hidden select
                    $('#dream_job_department_id').val(response.dream_job_department_id);
                    // Update custom select display text
                    let selectedText = $(`#dream_job_department_id option[value="${response.dream_job_department_id}"]`).text();
                    $('.custom-select-value').text(selectedText || 'Select Department');
                    $('#formModal').modal('show');
                } else {
            ...
            error: function(xhr) {
                if (xhr.status === 422) {
                    ...
                    $.each(errors, function(key, value) {
                        ...
                        if (key === 'dream_job_department_id') {
                            element.prev('.custom-select-container').find('.custom-select-value').addClass('is-invalid');
                        } else {
    ...
    $(document).on('click', '.delete-button', function() {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                let url = "{{ route('dreamJobPositionList.destroy', ':id') }}".replace(':id', id);
                $.ajax({
                    url: url,
                    type: 'POST',
